...
| 6 | The backend code doesn't decrypt content. | Read `backend/app.py`. `/v1/chat/message`, `/v1/memory/add`, `/v1/identity/init`, `/v1/content/rewrap` all accept v1 envelopes and store them verbatim — no crypto primitives called. The only place envelope bodies are decrypted is `backend/enclave_app.py`, which runs inside the TDX container. | 20 min |
| 7 | Identity.nudge can't silently mutate encrypted cards. | Read `backend/app.py`'s `identity_nudge`. On v1 cards it returns 409 with an explicit pointer to Phase C. Plain-text nudge only works on pre-v1 cards. | 2 min |
