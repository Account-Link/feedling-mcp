...
| `tools/call` returns 401 `user_not_found` (cloud mode) | User just ran Settings → Delete Account & Reset; the agent runtime is still pinned to the dead key | Ask the user to open Settings → Storage and copy the new MCP String, then re-run `claude mcp add feedling …` (or paste into Claude.ai / Hermes config). After re-import, your next `feedling_bootstrap` call will return `first_time` again — re-walk the onboarding. See also the **Re-auth + E2E verification** block below. |
